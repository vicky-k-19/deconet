...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10240,
        help="Redundancy factor. Default value =10*amb",
    )

    parser.add_argument("--meas", type=float, default=0.25, help="CS ratio")

    parser.add_argument("--a", type=float, default=0.7, help="Step size 1 parameter")

    parser.add_argument("--b", type=float, default=0.5, help="Step size 2 parameter")
...
def get_data_loaders(train_batch_size, val_batch_size):
    data_transform = Compose(
        [Grayscale(num_output_channels=1), ToTensor(), Normalize((0.5,), (0.5,))]
    )
...
def save_examples(model, val_loader, epoch, algo="ACF_CIFAR", device="cpu"):
    psnr_fn = PSNR()
    ...
    folder = f"results_ACF_CIFAR/{algo}_{date_fname()}_epoch.{epoch}"
    safe_mkdirs(folder)
...
if __name__ == "__main__":
    ...
    checkpoint_name = f"DECONET(CIFAR)-{ARGS.layers}L-red{ARGS.red}-lr{ARGS.lr}-mu{ARGS.mu}-init{ARGS.init}.pt"
